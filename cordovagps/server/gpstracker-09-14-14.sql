...
/*!50003 DROP PROCEDURE IF EXISTS `prcGetRoutesForUser` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE PROCEDURE `prcGetRoutesForUser`(_userName VARCHAR(50))
BEGIN
  CREATE TEMPORARY TABLE tempRoutesUser (
    sessionID VARCHAR(50),
    userName VARCHAR(50),
    startTime DATETIME,
    endTime DATETIME)
  ...
  INSERT INTO tempRoutesUser (sessionID, userName, startTime, endTime)
  SELECT DISTINCT sessionID, userName,  gpsTime, gpsTime
  FROM gpslocations where userName=_userName  group by sessionID;;

  UPDATE tempRoutesUser tr
  SET startTime = (SELECT MIN(gpsTime) FROM gpslocations gl
  WHERE gl.sessionID = tr.sessionID
  AND gl.userName = tr.userName);
